...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='convert between the note name and frequency'
    )
    parser.add_argument(
        'note_spec',
        nargs='*',
        help="""specify the octave and the note
                        or frequency.\n like 'O3A' / or '440'. \
                        The octave can be omitted (default O4).""",
    )
    parser.add_argument(
        '-o', '--octave', help='set the octave (this takes precedence)', default=4
    )
    ...
    if freq and note:
        parser.error(
            'both frequency and note name are supplied.\
    ...
    if note:
        try:
            ...
            re_octv, re_note = freq2note(f)
        except Exception as ex:
            parser.error(str(ex))
        if os.isatty(1):
            print(f'{re_note[0]}{re_octv} : {f:.4f} Hz')
        else:
            print(f'{f:.4f}')
    # freq -> note
    elif freq:
